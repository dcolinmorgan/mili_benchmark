...
def cambPredScore(indir='../../pc/redmo/data/MotifPipeline/camb_motif_pipeline_gamma5'): #,outdir='../../pc/redmo/data/MotifPipeline/camb_motif_pipeline_gamma5'):
    ...
    aurocs = pd.DataFrame()
    # os.makedirs(indir+'/tmp/',exist_ok=True)

    # if os.path.exists(indir+'/tmp'):
    #     shutil.rmtree(indir+'/tmp')
    os.makedirs(indir+'/tmp')
    ...
    #                  color='c', linestyle=':', linewidth=4)
            ...
            column.to_csv(indir+'/tmp/camb_PRE_window_methyl.txt',mode='a',header=False,index=False)
            aurocs=pd.concat([aurocs,column],axis=0)
            print("AUROC calculated for "+Col1+" in "+Col2)
    #         del roc_auc, roc_auc2


# def camb_plotPredScore(indir,measure='auroc'):
#     # table2=table
#     # table.to_csv('data/MotifPipeline/PRE_overall.txt')
#     aurocs=pd.read_table(indir+'/tmp/camb_PRE_window_methyl.txt',sep=',',usecols=[1,2,3,4,5],names=['cell','TF','mo_auroc','me_auroc','wgbs_auroc'])
#     aurocs=aurocs[aurocs.cell!='0']
    ...
    try:
        heat.columns=['TF','pwm-A549','pwm-GM12878','pwm-HeLa','pwm-HepG2','pwm-K562','pwm-SKNSH',
        ...
        plt.savefig(indir+"/tmp/camb_"+measure+"_heat.png",dpi=300,bbox_inches = "tight")
        plt.show
        ...
        if measure=='auroc':
            ...
            g=sns.swarmplot(x='data - cell line',y='AUPR', data=meltbox,
                          size=2, color=".3", linewidth=0)
        g.set(ylim=(0, 1))
        plt.savefig(indir+"/tmp/camb_"+measure+"_box.png",dpi=300,bbox_inches = "tight")
        ...
        df_ttest.to_csv(indir+"/tmp/camb_"+measure+"_ttest.txt")
        meltbox.to_csv(indir+"/tmp/camb_"+measure+"_meltbox.txt",sep='\t')
    except:
        pass
