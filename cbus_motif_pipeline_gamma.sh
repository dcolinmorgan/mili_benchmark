...
do
  ...
  eval "~/../rekrg/Tools/bedtools2/bin/bedtools intersect  -wa -wb -a data/MotifPipeline/ENCODE/wgbsin/"$cell"both.txt -b data/MotifPipeline/ENCODE/methyl_array/"$cell"_MeArrayHG38e.txt" |cut -f1,2,3,4,5,9,10,11 > $bench/overlap_tmpA_$cell"_"$buffer$out_fmt
  ...
  # cut -f1,2,3,4,5,6,7,8,9,13 $bench/overlap_tmpB_$cell"_"$buffer$out_fmt > $bench/overlap_tmpBB_$cell"_"$buffer$out_fmt
  awk '{print $1,$2,$3,$4,$5,$6,$13,$7,$8,$9}' OFS='\t' $bench/overlap_tmpB_$cell"_"$buffer$out_fmt > $bench/overlap_tmpBB_$cell"_"$buffer$out_fmt
