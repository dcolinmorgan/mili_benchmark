...
def predScore(indir='data/MotifPipeline/sthlm_motif_5_QCbeta/red',outdir='data/MotifPipeline/sthlm_motif_5_QCbeta/red/test',agg_meth='mean',cell=None,TF=None,region=None,depth=None):
    ...
    for jac,trace in enumerate(traces):
        ...
        if np.sum(data.ChIPTF)>5:
            print("calculating prediction accuracy for "+Col2+' in '+Col1)
    #     if not data.empty:
    #         data=data[np.abs(data.c2-data.c3)<=1000]
        ...
        #         
                ...
                roc_auc5_tr=metrics.auc(fpr, tpr)
                plt.plot(fpr, tpr, 
                ...
                precision, recall, thresholds = metrics.precision_recall_curve(data.ChIPTF, rwgp)
                rand_auc4=metrics.average_precision_score(data.ChIPTF, rwgp)
                plt.plot(recall,precision, 
                         label='shuf WGBS ({0:0.2f})'
                               ''.format(rand_auc4),
                         color='xkcd:light blue', linestyle='-', linewidth=2)
                ...
                precision, recall, thresholds = metrics.precision_recall_curve(data.ChIPTF, rmep)
                rand_auc6=metrics.average_precision_score(data.ChIPTF, rmep)
                plt.plot(recall,precision, 
                         label='shuf MeArray ({0:0.2f})'
                               ''.format(rand_auc6),
                         color='xkcd:light orange', linestyle='-', linewidth=2)
                precision, recall, thresholds = metrics.precision_recall_curve(dataxxx.ChIPTF, dataxxx.wgbs)
                roc_auc4_tr=metrics.average_precision_score(dataxxx.ChIPTF, dataxxx.wgbs)
                plt.plot(recall,precision, 
                         label='threshold WGBS ({0:0.2f})'
                               ''.format(roc_auc4_tr),
                         color='xkcd:red', linestyle='-', linewidth=2)
        ...
        #         mcc3= np.divide((tp3* tn3 - fp3 * fn3), np.sqrt((tp3+fp3)*(tp3+fn3)*(tn3+fp3)*(tn3+fn3)))
            ...
            #     Col2=Col.split('_')[1] 
                ...
                Coll1=rand_auc #motif rand auroc
                Coll2=rand_auc2 #wgbs rand auroc
                Coll3=rand_auc5 #array rand auroc
                ...
                Coll10=roc_auc6_tr #array threshold aupr
        #         Col7=mcc1 #motif mcc
        #         Col8=mcc2 #wgbs mcc
        #         Col12=mcc3 #mearray mcc
                Col9=np.nanmean(table2) #mean distance/difference
                Col13=np.nanmean(tbl)
                Col17=np.nanmean(tblC)
                Col14=np.nansum(tmpTBL2) # hits
                Col15=np.nansum(tmpTBL)
                Col18=np.nansum(tmpTBLC)
                Col16=data.size
